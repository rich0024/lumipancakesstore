...
export default function Admin() {
  ...
  const handleUpdateCard = async (id: number, cardData: Partial<Photocard>) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${process.env.API_URL || '${process.env.API_URL || 'http://localhost:3001'}'}/api/admin/photocards/${id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleUpdatePrint = async (id: number, printData: Partial<Print>) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${process.env.API_URL || '${process.env.API_URL || 'http://localhost:3001'}'}/api/admin/prints/${id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleBulkDeletePhotocards = async (ids: number[]) => {
    try {
      ...
      const deletePromises = ids.map(id => 
        fetch(`${process.env.API_URL || '${process.env.API_URL || 'http://localhost:3001'}'}/api/admin/photocards/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const handleBulkDeletePrints = async (ids: number[]) => {
    try {
      const token = localStorage.getItem('authToken');
      const deletePromises = ids.map(id => 
        fetch(`${process.env.API_URL || '${process.env.API_URL || 'http://localhost:3001'}'}/api/admin/prints/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
